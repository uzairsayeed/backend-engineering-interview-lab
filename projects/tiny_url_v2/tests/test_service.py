...
def test_create_url_with_expiration() -> None:
    ...
    assert result.expires_at == (FIXED_TIME + timedelta(hours=1))
...
def test_create_url_rejects_reserved_custom_code() -> None:
    repository = ShortUrlRepository()
    ...
    with pytest.raises(ReservedShortCodeError) as error:
        service.create_url(
            destination_url="https://example.com",
            custom_code="health",
        )

    assert error.value.short_code == "health"


def test_generated_code_retries_reserved_code() -> None:
    repository = ShortUrlRepository()

    generated_codes = iter(["health", "abc123"])
...
def test_reserved_code_check_is_case_insensitive() -> None:
    repository = ShortUrlRepository()
    ...
    with pytest.raises(ReservedShortCodeError):
        service.create_url(
            destination_url="https://example.com",
            custom_code="Health",
        )
