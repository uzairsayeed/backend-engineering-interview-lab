...
logger = logging.getLogger("tinyurl.service")
...
from app.repository_protocol import (
...
class ShortUrlService:
    ...
    def __init__(
        ...
        self._reserved_short_codes = frozenset(
            code.casefold()
            for code in reserved_short_codes
        )
    ...
    def create_url(
        ...
        if custom_code is not None:
            if self._is_reserved_short_code(custom_code):
                raise ReservedShortCodeError(custom_code)
            ...
            saved_url = self._repository.save(short_url)

            logger.info(
                "short_url_created short_code=%s code_source=custom has_expiration=%s",
                saved_url.short_code,
                saved_url.expires_at is not None,
            )
        ...
        for _ in range(self._max_generation_attempts):
            ...
            if self._is_reserved_short_code(generated_code):
                logger.debug(
                    "generated_short_code_reserved short_code=%s",
                    generated_code,
                )
                continue
            ...
            try:
                ...
                logger.debug(
                    "generated_short_code_collision short_code=%s",
                    generated_code,
                )
                continue
            ...
            logger.info(
                "short_url_created "
                "short_code=%s code_source=generated "
                "has_expiration=%s",
                saved_url.short_code,
                saved_url.expires_at is not None,
            )
        ...
        logger.error(
            "short_code_generation_exhausted attempts=%s",
            self._max_generation_attempts,
        )

        raise ShortCodeGenerationError(self._max_generation_attempts)

    def resolve_url(
        self,
        short_code: str,
    ) -> ShortUrl:
        short_url = self._repository.get(
            short_code
        )
        ...
        # Why check for missing twice?
            # The sequence is:
                # 1. Retrieve URL
                # 2. Validate expiration
                # 3. Increment count
            # Between steps 1 and 3, another transaction could theoretically delete the row.
        ...
        logger.debug(
            "short_url_resolved "
            "short_code=%s redirect_count=%s",
            updated_short_url.short_code,
            updated_short_url.redirect_count,
        )
    ...
    def delete_url(
        ...
        was_deleted = self._repository.delete(short_code)

        if not was_deleted:
            raise ShortCodeNotFoundError(short_code)

        logger.info(
            "short_url_deleted short_code=%s",
            short_code,
        )
    ...
    def _calculate_expiration(
        ...
        if expires_in_seconds <= 0:
            raise InvalidExpirationError(expires_in_seconds)

        return created_at + timedelta(seconds=expires_in_seconds)

    def _is_reserved_short_code(
        self,
        short_code: str,
    ) -> bool:
        return short_code.casefold() in self._reserved_short_codes
